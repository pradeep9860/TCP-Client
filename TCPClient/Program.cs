...
namespace TCPClient
{
    static class Program
    {
        ...
        private static void StackTrashParshedRecord(Reader.ReaderMethod reader)
        {
            ...
            OperateTagISO18000Buffer m_curOperateTagISO18000Buffer = new OperateTagISO18000Buffer();
        ...
        private static void AnalyData(Reader.MessageTran msgTran)
        {
        ...
        private static void ProcessInventoryReal(Reader.MessageTran msgTran)
        {
            ...
            if (msgTran.AryData.Length == 1)
            {
                ...
                RefreshInventoryReal(0x00, m_curInventoryBuffer);
                RunLoopInventroy(m_curInventoryBuffer, reader);
            }
            else if (msgTran.AryData.Length == 7)
            {
                ...
                RefreshInventoryReal(0x01,  m_curInventoryBuffer);
                RunLoopInventroy(m_curInventoryBuffer, reader);
            }
            else
            {
               // m_nTotal++;
                ...
                RefreshInventoryReal(0x89, m_curInventoryBuffer);
            }
        }
        ...
        private static void RefreshInventoryReal(byte btCmd, InventoryBuffer m_curInventoryBuffer)
        { 
            if (false)
            {
        ...
        private static void RunLoopInventroy(InventoryBuffer m_curInventoryBuffer, Reader.ReaderMethod reader)
        {
            ReaderSetting m_curSetting = new ReaderSetting();
            if (false)
            {
